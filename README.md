...
未パッケージの拡張機能は、読み込み直すたびに拡張機能IDが変わります。IDを固定したい場合は`manifest.json`に`key`を追加してください。
...
`lib/asin.js`は`chrome.*`のAPIに依存しないため、ブラウザを起動せずにURL正規化のロジックを検証できます。

## 免責

本拡張機能は個人が開発した非公式のツールであり、Amazon.com, Inc.およびその関連会社とは一切関係がありません。`Amazon`は Amazon.com, Inc. またはその関連会社の商標です。
