...
/**
 * Amazon側のJavaScriptがページ読み込み後に自動で付与するクエリパラメータのキー一覧。
 *
 * `/dp/<ASIN>`を開くと、Amazonのクライアントサイドスクリプトが商品バリエーションの
 * 選択状態として`?th=1`（`?psc=1`を伴うことがある）を付ける。これらを未正規化と
 * みなすと、アイコンを押すたびに同じ内容のページを読み込み直すことになる。
 * 除去しても即座に付け直されるため、正規化の判定では無視する。
 * @type {readonly string[]}
 */
export const AUTO_ADDED_QUERY_KEYS = ["th", "psc"];
 ...
 * クエリ・フラグメントの有無で判定する。
 *
 * クエリのうち`AUTO_ADDED_QUERY_KEYS`のキーだけは無視する。それ以外のキーが
 * 1つでもあれば未正規化として扱い、`ref=`やアフィリエイトタグを除去する。
 * @param {URL} u 対象URL
...
export function isAlreadyNormalized(u, asin) {
  ...
  for (const key of u.searchParams.keys()) {
    if (!AUTO_ADDED_QUERY_KEYS.includes(key)) return false;
  }
